...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id not in USER_STATE:
        ...
        # Track time & batch behavior
        last_time = USER_STATE[user_id].get("last_apk_time", 0)
        if now - last_time < 1:
            USER_STATE[user_id]["apk_arrivals"] += 1
        else:
            USER_STATE[user_id]["apk_arrivals"] = 1
        USER_STATE[user_id]["last_apk_time"] = now

    state = USER_STATE[user_id]

    if any(apk["file_id"] == doc.file_id for apk in state["apks"]):
    ...
    is_batch_upload = state["apk_arrivals"] >= 2  # 2+ APKs within 1s → batch

    if is_batch_upload:
        state["pending_apks"].append(apk_info)
        ...
        await update.message.reply_text(
            f"⏳ APK {apk_index} received.\nNow send the Key to continue."
        )
        return

    # Not batch → handle normally
