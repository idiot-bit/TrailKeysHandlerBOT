...
OWNER_ID = config_data.get("owner_id", int(os.getenv("OWNER_ID", "123456789")))
ALLOWED_USERS = set(config_data.get("allowed_users", []))
USER_DATA = config_data.get("user_data", {})
CONFIG = {
    "setups": config_data.get("setups", {
        "1": {"source": "", "dest": "", "caption": ""},
        "2": {"source": "", "dest": "", "caption": ""},
        "3": {"source": "", "dest": "", "caption": ""}
    })
}
...
def save_config():
    with open("config.json", "w") as f:
        json.dump({
            ...
            "user_data": USER_DATA,
            "setups": CONFIG.get("setups", {})  # ← new addition for source/dest/captions
        }, f, indent=4)
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id == OWNER_ID:
        await update.message.reply_text(
            ...
            "🛠️ *Setup 1*\n"
            "/setsource1 - Set Source Channel\n"
            "/setdest1 - Set Destination Channel\n"
            "/setcaption1 - Set Caption\n"
            "/resetsetup1 - Reset Setup 1\n\n"
            "🛠️ *Setup 2*\n"
            "/setsource2 - Set Source Channel\n"
            "/setdest2 - Set Destination Channel\n"
            "/setcaption2 - Set Caption\n"
            "/resetsetup2 - Reset Setup 2\n\n"
            "🛠️ *Setup 3*\n"
            "/setsource3 - Set Source Channel\n"
            "/setdest3 - Set Destination Channel\n"
...
@bot.on_message(filters.private & filters.command(["setsource1", "setsource2", "setsource3"]) & filters.user(OWNER_ID))
async def set_source(bot, msg):
    setup = msg.command[0][-1]
    CONFIG["setups"][setup]["source"] = msg.text.split(None, 1)[1].strip()
    save_config()
    await msg.reply(f"✅ Source {setup} → `{CONFIG['setups'][setup]['source']}`", quote=True)

@bot.on_message(filters.private & filters.command(["setdest1", "setdest2", "setdest3"]) & filters.user(OWNER_ID))
async def set_dest(bot, msg):
    setup = msg.command[0][-1]
    CONFIG["setups"][setup]["dest"] = msg.text.split(None, 1)[1].strip()
    save_config()
    await msg.reply(f"✅ Destination {setup} → `{CONFIG['setups'][setup]['dest']}`", quote=True)

@bot.on_message(filters.private & filters.command(["setcaption1", "setcaption2", "setcaption3"]) & filters.user(OWNER_ID))
async def set_caption(bot, msg):
    setup = msg.command[0][-1]
    caption = msg.text.split(None, 1)[1]
    if "Key -" not in caption:
        await msg.reply("⚠️ Caption must contain `Key -` placeholder.")
        return
    CONFIG["setups"][setup]["caption"] = caption
    save_config()
    await msg.reply(f"✅ Caption {setup} saved!", quote=True)

@bot.on_message(filters.channel & filters.document)
async def auto_forward_apk(client, msg):
    if not msg.document.file_name.endswith(".apk"):
        return
    ...
    incoming_username = f"@{msg.chat.username}" if msg.chat.username else None

    for setup_number, setup in CONFIG["setups"].items():
        if not all(setup.values()):
            continue

        # Match channel
        if setup["source"] not in [incoming_chat_id, incoming_username]:
            continue
        ...
        final_caption = setup["caption"].replace("Key -", f"Key - `{key}`")
        ...
        try:
            ...
            await client.send_message(
                OWNER_ID,
                f"*Completed Process! :*\n"
                f"• Source Picked → `{setup['source']}`\n"
                f"• Desi Sended → `{setup['dest']}`\n"
                f"• Sended Key → `Key - {key}`",
                parse_mode="Markdown"
            )
        except Exception as e:
            await client.send_message(OWNER_ID, f"❌ Error: {str(e)}")
        break
        
@bot.on_message(filters.private & filters.command(["resetsetup1", "resetsetup2", "resetsetup3"]) & filters.user(OWNER_ID))
async def reset_setup(_, msg):
    setup = msg.command[0][-1]
    CONFIG["setups"][setup] = {"source": "", "dest": "", "caption": ""}
    save_config()
    await msg.reply(f"♻️ Setup {setup} has been reset.")

@bot.on_message(filters.private & filters.command("viewsetup") & filters.user(OWNER_ID))
async def view_setup(_, msg):
    txt = ""
    for i, s in CONFIG["setups"].items():
        if all(s.values()):
            txt += (
                f"*Configured setup {i}:*\n"
                f"• Source Channel {i} → `{s['source']}`\n"
                f"• Desi Channel {i}  → `{s['dest']}`\n"
                f"• Set Caption {i}  → Caption Saved !\n\n"
            )
    await msg.reply(txt or "⚠️ No setups configured.", parse_mode="Markdown")
