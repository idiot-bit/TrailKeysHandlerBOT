...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Prevent duplicate handling of the same APK
    ...
    if match:
        ...
        # Key is missing — defer confirmation
        state["status"] = "waiting_key"
        state["current_apk"] = None  # batch mode
        state.setdefault("pending_apks", [])

        # Prevent duplicate pending_apks entry
        if not any(apk["file_id"] == doc.file_id for apk in state["pending_apks"]):
            state["pending_apks"].append(apk_info)

        # Start confirmation task only once
