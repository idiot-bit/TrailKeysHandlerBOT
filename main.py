...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if match:
        ...
        if not saved_caption or not channel_id:
            buttons = []
            if not channel_id:
                buttons.append([InlineKeyboardButton("📡 Set Channel ID", callback_data="set_channel_id")])
            ...
            await update.message.reply_text(
                ...
                "Clicking 🎯\n"
                "📡 *Set Channel Id* → 𝖺𝖽𝖽 𝖸𝗈𝗎𝗋 𝖢𝗁𝖺𝗇𝗇𝖾𝗅\n"
                "📝 *Set Caption* → 𝖠𝖽𝖽 𝖸𝗈𝗎𝗋 𝖪𝖾𝗒 𝖢𝖺𝗉𝗍𝗂𝗈𝗇\n\n"
...
def main():
    ...
    application.add_handler(CallbackQueryHandler(handle_callback_query, pattern="^(set_channel_id|set_caption)$"))
