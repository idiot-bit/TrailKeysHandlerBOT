...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not update.effective_user:
        return  # Ignore updates that aren't from a user
    ...
    for route, data in ROUTES.items():
        try:  # ← FIX this indentation
            source = data["source"]
            destination = data["destination"]
            caption_template = data["caption"]

            # Match by file size
            if (
                (route == "1" and 1 <= file_size_mb <= 50) or
                (route == "2" and 80 <= file_size_mb <= 2048) or
                (route == "3")
            ):
                # Check all required
                missing = []
                if not source: missing.append("/sourcechannel" + route)
                if not destination: missing.append("/desichannel" + route)
                if "Key -" not in caption_template: missing.append("/setcaption" + route)

                if missing:
                    await update.message.reply_text(
                        f"⚠️ Missing setup for Route {route}:\n" + "\n".join(missing)
                    )
                    return
                ...
                await update.message.reply_text(
                    f"*Completed Process! :*\n"
                    f" • Source Picked → {source}\n"
                    f" • Desi Sended → {destination}\n"
                    f" • Sended Key → {key}",
                    parse_mode="Markdown"
                )
                return
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state["status"] == "waiting_key":
        ...
        route = state["status"].split("_")[-1]
        caption = update.message.text.strip()
        if "Key -" not in caption:
            await update.message.reply_text("❌ Caption must include `Key -` placeholder.")
        else:
            ROUTES[route]["caption"] = caption
            save_config()
            await update.message.reply_text(f"✅ Caption saved for Route {route}")
            del USER_STATE[user_id]
...
async def set_source_channel(update: Update, context: ContextTypes.DEFAULT_TYPE, route: str):
    if update.effective_user.id != OWNER_ID:
        return
    if not context.args:
        await update.message.reply_text(f"❗ Send a valid source channel (ex: @channel or -100id)")
        return
    ROUTES[route]["source"] = context.args[0]
    save_config()
    await update.message.reply_text(f"✅ Source channel {route} saved!")

async def set_destination_channel(update: Update, context: ContextTypes.DEFAULT_TYPE, route: str):
    if update.effective_user.id != OWNER_ID:
        return
    if not context.args:
        await update.message.reply_text(f"❗ Send a valid destination channel (ex: @channel or -100id)")
        return
    ROUTES[route]["destination"] = context.args[0]
    save_config()
    await update.message.reply_text(f"✅ Destination channel {route} saved!")

async def set_route_caption(update: Update, context: ContextTypes.DEFAULT_TYPE, route: str):
    if update.effective_user.id != OWNER_ID:
        return
    USER_STATE[OWNER_ID] = {"status": f"waiting_caption_{route}"}
    await update.message.reply_text(f"📝 Send caption for Route {route} (must include 'Key -')")
        
async def reset_setup(update: Update, context: ContextTypes.DEFAULT_TYPE, route: str):
    if update.effective_user.id != OWNER_ID:
        return
    ROUTES[route] = {"source": "", "destination": "", "caption": ""}
    save_config()
    await update.message.reply_text(f"♻️ Route {route} config reset!")
...
async def view_setup(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = ""
    for r in ["1", "2", "3"]:
        src = ROUTES[r].get("source", "Not set")
        dst = ROUTES[r].get("destination", "Not set")
        cap = "Caption Saved!" if ROUTES[r].get("caption") else "Not set"
        msg += f"*Configured Routes setup {r}:*\n"
        msg += f" • Source Channel {r} → {src}\n"
        msg += f" • Desi Channel {r}  → {dst}\n"
        msg += f" • Set Caption {r}  → {cap}\n\n"
...
def main():
    ...
    app.add_handler(CommandHandler("sourcechannel1", lambda u, c: set_source_channel(u, c, "1")))
    app.add_handler(CommandHandler("sourcechannel2", lambda u, c: set_source_channel(u, c, "2")))
    app.add_handler(CommandHandler("sourcechannel3", lambda u, c: set_source_channel(u, c, "3")))
    app.add_handler(CommandHandler("desichannel1", lambda u, c: set_destination_channel(u, c, "1")))
    app.add_handler(CommandHandler("desichannel2", lambda u, c: set_destination_channel(u, c, "2")))
    app.add_handler(CommandHandler("desichannel3", lambda u, c: set_destination_channel(u, c, "3")))
    app.add_handler(CommandHandler("setcaption1", lambda u, c: set_route_caption(u, c, "1")))
    app.add_handler(CommandHandler("setcaption2", lambda u, c: set_route_caption(u, c, "2")))
    app.add_handler(CommandHandler("setcaption3", lambda u, c: set_route_caption(u, c, "3")))
