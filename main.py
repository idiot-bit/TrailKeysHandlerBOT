...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not doc.file_name.endswith(".apk"):
        await update.message.reply_text(
            "⚠️ *Only APK files are allowed\\!* 📦❌ Let's keep it real\\! 🔥",
            parse_mode="MarkdownV2"
        )
        return
    ...
    if match:
        ...
        # Check if caption or channel ID is missing
        if not saved_caption or not channel_id:
            buttons = []
            if not channel_id:
                buttons.append([InlineKeyboardButton("📡 Set Channel ID", callback_data="get_channel_id")])
            if not saved_caption:
                buttons.append([InlineKeyboardButton("📝 Set Caption", callback_data="get_caption")])

            await update.message.reply_text(
                "⚙️ *First Setup Your Bot* \n\n"
                "• /setchannelid → 𝖺𝖽𝖽 𝖸𝗈𝗎𝗋 𝖢𝗁𝖺𝗇𝗇𝖾𝗅 📡\n"
                "• /setcaption → 𝖠𝖽𝖽 𝖸𝗈𝗎𝗋 𝖪𝖾𝗒 𝖢𝖺𝗉𝗍𝗂𝗈𝗇 📝\n\n"
                "✅ *This Setup Complete Then*\n"
                "🚀 *Share apk !!*",
                parse_mode="MarkdownV2",
        ...
        await update.message.reply_text(
            "𝖩𝗎𝗌𝗍 𝖳𝗒𝗉𝖾 𝗍𝗁𝖾 𝖪𝖾𝗒 🔑 𝗍𝗁𝖾𝗇 𝖲𝖾𝗇𝖽\n"
            "𝖠𝗎𝗍𝗈𝗆𝖺𝗀𝗂𝖼𝖺𝗅𝗅𝗒 𝗍𝗁𝖺𝗍 𝗄𝖾𝗒 🎯\n"
            "𝗐𝗂𝗅𝗅 𝖻𝖾 𝖺𝖽𝖽𝖾𝖽 𝗍𝗈 𝗒𝗈𝗎𝗋 𝖢𝖺𝗉𝗍𝗂𝗈𝗇 🪄",
            parse_mode="MarkdownV2"
        )
