...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state["status"] == "waiting_key":
        ...
        if pending_apks:
            total = len(pending_apks)
            for i, apk in enumerate(pending_apks):
                apk["key"] = key
                if total == 1:
                    apk["caption"] = user_caption.replace("Key -", key_line)
                elif total == 2:
                    if i == 0:
                        apk["caption"] = key_line
                    else:
                        apk["caption"] = user_caption.replace("Key -", key_line)
                elif total == 3:
                    if i < 2:
            ...
            await update.message.reply_text(f"✅ Key added to {total} APK(s).", reply_markup=get_apk_share_keyboard())
        else:
            # Single mode
            apk = state.get("current_apk")
            if not apk:
                await update.message.reply_text("⚠️ Please upload the APK first.")
                return
            ...
            state["status"] = "waiting_apk"
            await update.message.reply_text("✅ Key added.", reply_markup=get_apk_share_keyboard())
