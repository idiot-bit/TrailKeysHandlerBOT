...
async def countdown_and_check(user_id, chat_id, context):
    try:
        for remaining in range(5, 0, -1):
            ...
            if message_id and session_files:
                try:
                    await context.bot.edit_message_text(
                        chat_id=chat_id,
                        message_id=message_id,
                    ...
                    print(f"Countdown edit error (sec {remaining}): {e}")

        # After countdown ends, ask for key
        state = USER_STATE.get(user_id, {})
        message_id = state.get("progress_message_id")
        session_files = state.get("session_files", [])

        if session_files and message_id:
            try:
                await context.bot.edit_message_text(
                    chat_id=chat_id,
                    message_id=message_id,
                ...
                print(f"Countdown final key ask error: {e}")

            USER_STATE[user_id]["waiting_key"] = True
            USER_STATE[user_id]["progress_message_id"] = None

    except Exception as e:
        print(f"Countdown overall error: {e}")
...
async def method2_edit_caption(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        print(f"Error while editing preview after caption: {e}")
...
async def method2_convert_quote(user_id, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Mark quote_applied = True to HIDE Quote button next time
    ...
    try:
        await context.bot.edit_message_text(
            ...
            message_id=preview_message_id,
            text=text,
            parse_mode="HTML",  # Important to render <code> formatting
...
async def method2_convert_mono(user_id, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Mark mono_applied = True to HIDE Normal Key button next time
    USER_STATE[user_id]["mono_applied"] = True

    buttons = build_method2_buttons(user_id)  # Rebuild buttons dynamically
    ...
    try:
        await context.bot.edit_message_text(
            ...
            message_id=preview_message_id,
            text=text,
            parse_mode="Markdown",  # Important to render `mono` style
            reply_markup=buttons
        )
    except Exception as e:
...
def build_method2_buttons(user_id):
    ...
    if not USER_STATE.get(user_id, {}).get("quote_applied"):
        buttons.append([InlineKeyboardButton("✍️ Quote Key", callback_data="method2_quote")])

    if not USER_STATE.get(user_id, {}).get("mono_applied"):
        buttons.append([InlineKeyboardButton("🔤 Normal Key", callback_data="method2_mono")])

    buttons.append([InlineKeyboardButton("📝 Edit Caption", callback_data="method2_edit"),
                    InlineKeyboardButton("👁️ Show Preview", callback_data="method2_preview")])
