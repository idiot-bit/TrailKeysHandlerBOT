...
SOURCE_CHANNEL_RULES = {
    "sourcechannel1": {"min": 1 * 1024 * 1024, "max": 50 * 1024 * 1024},       # 1MB - 50MB
    "sourcechannel2": {"min": 80 * 1024 * 1024, "max": 2 * 1024 * 1024 * 1024}, # 80MB - 2GB
    "sourcechannel3": {"min": 0, "max": float("inf")}  # fallback
    }
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id == OWNER_ID:
        await update.message.reply_text(
            ...
            "/sourcechannel1 - Set source 1\n"
            "/sourcechannel2 - Set source 2\n"
            "/sourcechannel3 - Set source 3\n"
            "/desichannel1 - Set destination 1\n"
            "/desichannel2 - Set destination 2\n"
            "/desichannel3 - Set destination 3\n"
            "/targetcaption1 - Set caption for target 1\n"
            "/targetcaption2 - Set caption for target 2\n"
            "/targetcaption3 - Set caption for target 3"
        )
...
async def set_channel_mapping(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    if user_id != OWNER_ID:
        return

    command = update.message.text.split()[0].lstrip('/')
    if command not in CHANNEL_MAPPINGS:
        return

    value = ' '.join(context.args)
    if not value:
        await update.message.reply_text(f"⚠️ Usage: /{command} <@channel or -100id>", parse_mode="Markdown")
        return

    CHANNEL_MAPPINGS[command] = value
    save_config()
    await update.message.reply_text(f"✅ `{command}` updated:\n→ `{value}`", parse_mode="Markdown")
        
def get_channel_rule_by_size(file_size):
    for source_key, rule in SOURCE_CHANNEL_RULES.items():
        if rule["min"] <= file_size <= rule["max"]:
            return source_key
    return None        
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if match:
        ...
        matched_source = get_channel_rule_by_size(file_size)

        if not matched_source:
            await update.message.reply_text("❌ No matching source rule found for this file size.")
            return

        # Determine which target caption and destination channel to use
        caption_key = matched_source.replace("sourcechannel", "targetcaption")
        desi_key = matched_source.replace("sourcechannel", "desichannel")

        target_caption = CHANNEL_MAPPINGS.get(caption_key, "")
        desi_channel = CHANNEL_MAPPINGS.get(desi_key, "")

        if not target_caption or not desi_channel:
            await update.message.reply_text(f"⚠️ Missing caption or channel mapping for `{matched_source}`.", parse_mode="Markdown")
            return

        final_caption = target_caption.replace("Key -", f"Key - `{key}`")
            await context.bot.send_document(
            chat_id=desi_channel,
        ...
        await update.message.reply_text(
            "✅ *Done\\!* Your file was shared successfully\\.\n"
            "It went straight to the destination channel\\! 🚀",
            parse_mode="MarkdownV2"
        )
        ...
        if user_id != OWNER_ID:
            ...
            chat_id=OWNER_ID,
            text=f"📤 File shared by [{update.effective_user.first_name}](tg://user?id={user_id}) → {desi_channel}",
            parse_mode="Markdown"
        )
        
        SHARED_FILES = set()
        if doc.file_id in SHARED_FILES:
            await update.message.reply_text("⚠️ This file was already shared.")
            return
        SHARED_FILES.add(doc.file_id)
...
def main():
    for cmd in [
        "sourcechannel1", "sourcechannel2", "sourcechannel3",
        "desichannel1", "desichannel2", "desichannel3",
        "targetcaption1", "targetcaption2", "targetcaption3"
    ]:
    app.add_handler(CommandHandler(cmd, set_channel_mapping))
    app = Application.builder().token(BOT_TOKEN).build()
