...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if match:
        ...
        if not saved_caption or not channel_id:
            buttons = []
            if not channel_id:
                buttons.append([InlineKeyboardButton("📡 Set Channel ID", switch_inline_query_current_chat="/setchannelid")])
            if not saved_caption:
                buttons.append([InlineKeyboardButton("📝 Set Caption", switch_inline_query_current_chat="/setcaption")])
