...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    user_id = query.from_user.id
    data = query.data  # <<< Always FETCH data first!!

    # New: Method 2 handler first
    ...
    if data == "share_no":
        await query.edit_message_text(
            "🙅‍♂️ *No worries!* You can retry anytime. Just drop your .apk again. 🚀",
            parse_mode="Markdown"
        )
        return

    if data == "get_channel_id":
        USER_STATE[user_id] = {"status": "waiting_channel"}
        await query.edit_message_text(
            "🔧 *Setup Time\\!*\n"
            "Send me your Channel ID now\\. 📡\n"
    ...
    # Set Channel and Caption for Method 2
