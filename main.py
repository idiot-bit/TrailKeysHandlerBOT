...
async def auto_handle_channel_post(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # --- NEW PART: Wait 20 seconds
    await asyncio.sleep(20)

    # After 20 seconds, double check if message still exists
    try:
        current_message = await context.bot.get_chat(chat_id)
    except Exception as e:
        print(f"❌ Failed to fetch chat after waiting: {e}")
        return  # Skip if channel inaccessible
