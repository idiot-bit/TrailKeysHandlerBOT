...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id == OWNER_ID:
        await update.message.reply_text(
            ...
            "Setup 1:\n"
            "/sourcechannel1 - Set Source\n"
            "/desichannel1 - Set Destination\n"
            "/setcaption1 - Set Caption\n"
            "/resetsetup1 - Reset Setup 1\n\n"
            "Setup 2:\n"
            "/sourcechannel2 - Set Source\n"
            "/desichannel2 - Set Destination\n"
            "/setcaption2 - Set Caption\n"
            "/resetsetup2 - Reset Setup 2\n\n"
            "Setup 3:\n"
            "/sourcechannel3 - Set Source\n"
            "/desichannel3 - Set Destination\n"
            "/setcaption3 - Set Caption\n"
            "/resetsetup3 - Reset Setup 3\n\n"
            "/viewsetup - View All Configured Setups"
        )
...
async def set_source_channel(update: Update, context: ContextTypes.DEFAULT_TYPE, setup_number: str):
    if update.effective_user.id != OWNER_ID:
        return
    if not context.args:
        await update.message.reply_text(f"❌ Use like this: `/sourcechannel{setup_number} @sourcechannel`", parse_mode="Markdown")
        return
    SETUPS[setup_number]["source_channel"] = context.args[0]
    save_config()
...
async def set_destination_channel(update: Update, context: ContextTypes.DEFAULT_TYPE, setup_number: str):
    if update.effective_user.id != OWNER_ID:
        return
    if not context.args:
        await update.message.reply_text(f"❌ Use like this: `/desichannel{setup_number} @destinationchannel`", parse_mode="Markdown")
        return
    SETUPS[setup_number]["destination_channel"] = context.args[0]
    save_config()
    if all(SETUPS[setup_number].values()):
        await update.message.reply_text(
            f"*Completed Setup {setup_number}:*\n"
...
async def set_caption_for_setup(update: Update, context: ContextTypes.DEFAULT_TYPE, setup_number: str):
    if update.effective_user.id != OWNER_ID:
        return
    if not context.args:
        await update.message.reply_text(f"❌ Use like this: `/setcaption{setup_number} Your caption with Key - placeholder`", parse_mode="Markdown")
        return
    caption = " ".join(context.args)
    if "Key -" not in caption:
        await update.message.reply_text("⚠️ Caption must include `Key -` placeholder.", parse_mode="Markdown")
        return
...
async def view_setup(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = ""
    for num, setup in SETUPS.items():
        if all(setup.values()):
            msg += (
                f"*Configured setup {num}:*\n"
                f"• Source Channel {num} → `{setup['source_channel']}`\n"
                f"• Desi Channel {num}  → `{setup['destination_channel']}`\n"
...
async def monitor_channels(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    channel_id = str(update.message.chat.username or update.message.chat.id)

    key_match = re.search(r'Key\s*-\s*(\S+)', caption)
    if not key_match:
        return  # No key found, ignore

    key = key_match.group(1)

    for setup_number, setup_data in SETUPS.items():
        source_channel = setup_data["source_channel"]
        destination_channel = setup_data["destination_channel"]
        caption_template = setup_data["caption"]

        if not (source_channel and destination_channel and caption_template):
            continue  # Skip incomplete setup

        if channel_id != source_channel.lstrip('@').lstrip('-100'):
            continue  # Not the matching source

        # Check file size range
        size_ok = (
            (setup_number == "1" and 1_000_000 <= file_size <= 50_000_000) or
            (setup_number == "2" and 80_000_000 <= file_size <= 2_000_000_000) or
            (setup_number == "3")  # Accept anything
        )
        if not size_ok:
            return
        ...
        if "Key -" not in caption_template:
            await context.bot.send_message(OWNER_ID, f"⚠️ Setup {setup_number} has a caption missing `Key -` placeholder.")
            return

        final_caption = caption_template.replace("Key -", f"`{key}`")
        ...
        try:
            ...
            await context.bot.send_message(
                ...
                text=(
                    f"*Completed Process! :*\n"
                    f"• Source Picked → `{source_channel}`\n"
                    f"• Desi Sended → `{destination_channel}`\n"
                    f"• Sended Key → `Key - {key}`"
                ),
                parse_mode="Markdown"
            )
        except Exception as e:
            await context.bot.send_message(OWNER_ID, f"❌ Failed to forward file for setup {setup_number}: {e}")
        return  # Only handle one matching setup
...
def main():
    ...
    app.add_handler(CommandHandler("setcaption1", lambda u, c: set_caption_for_setup(u, c, "1")))
    ...
    app.add_handler(CommandHandler("setcaption2", lambda u, c: set_caption_for_setup(u, c, "2")))
    ...
    app.add_handler(CommandHandler("setcaption3", lambda u, c: set_caption_for_setup(u, c, "3")))
    ...
    app.add_handler(MessageHandler(filters.Document.ALL, monitor_channels))
