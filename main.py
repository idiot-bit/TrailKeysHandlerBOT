...
async def confirm_apk_batch(user_id, context):
    await asyncio.sleep(1.0)  # Wait for all APKs to arrive

    state = USER_STATE.get(user_id)
    if not state:
        return

    pending = state.get("pending_apks", [])
    count = len(pending)

    if count > 0:
        await context.bot.send_message(
            chat_id=user_id,
            text=f"⏳ {count} APK{'s' if count > 1 else ''} received.\nNow send the Key to continue."
        )
...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Only allow key if it's the first APK and not a batch
    allow_key = len(state["apks"]) == 0 and not state.get("pending_apks")
    match = re.search(r'Key\s*-\s*(\S+)', caption) if allow_key else None

    apk_index = len(state["apks"]) + 1
    apk_info = {
        "file_id": doc.file_id,
        "key": "",
        "caption": "",
        "method": f"Method {apk_index}"
    }

    if match:
        detected_key = match.group(1)
        apk_info["key"] = detected_key

        saved_caption = USER_DATA.get(str(user_id), {}).get("caption", "")
        apk_info["caption"] = saved_caption.replace("Key -", f"Key - <code>{detected_key}</code>")

        await update.message.reply_text(
            f"✅ APK {apk_index} saved with key!",
            reply_markup=get_apk_share_keyboard()
        )
        state["apks"].append(apk_info)

    else:
        # Key missing or part of multi-apk batch — treat as batch
        state["status"] = "waiting_key"
        state["current_apk"] = None
        state.setdefault("pending_apks", []).append(apk_info)

        # Start confirmation only once
        if not state.get("confirm_task"):
            state["confirm_task"] = context.application.create_task(
                confirm_apk_batch(user_id, context)
            )


async def post_apks_to_channel(bot, channel_id, apk_list):
    if not apk_list:
        return

    # STEP 1: Send main caption (usually from the last APK)
    main_caption = apk_list[-1]["caption"]  # last one has full caption
    ...
    # STEP 2: Send each APK as a reply to the caption message
    for apk in apk_list:
