...
async def monitor_apks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    doc = update.message.document  # must come first
    if not doc.file_name.lower().endswith(".apk"):
        return  # Only process APKs
    ...
    for setup_number, setup in SETUPS.items():
        print(f"[Monitor] Incoming from {incoming_chat_id}, checking setup {setup_number}")  # ← correctly indented log
