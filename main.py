...
ROUTES = config.get("routes", {"1": {}, "2": {}, "3": {}})
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id == OWNER_ID:
        await update.message.reply_text(
            ...
            "/resetcaption - Reset saved caption 🧹\n"
            "/resetchannelid - Reset your channel ID 🔁\n"
            "/setchannelid - Set your Channel ID 📡\n"
            "/setcaption - Set your Caption ✍️\n\n"
            "🔄 Route Setup Commands:\n"
            "/sourcechannel1 - Set source for route 1\n"
            "/desichannel1 - Set destination for route 1\n"
            "/setcaption1 - Set caption for route 1\n"
            "/resetsetup1 - Reset route 1\n\n"
            "/sourcechannel2 - Set source for route 2\n"
            "/desichannel2 - Set destination for route 2\n"
            "/setcaption2 - Set caption for route 2\n"
            "/resetsetup2 - Reset route 2\n\n"
            "/sourcechannel3 - Set source for route 3\n"
            "/desichannel3 - Set destination for route 3\n"
            "/setcaption3 - Set caption for route 3\n"
            "/resetsetup3 - Reset route 3\n"
            "/viewsetup - View all route setups"
        )
...
async def set_source_channel1(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ROUTES["1"]["source"] = get_chat_id(update)
    save_config()
    await update.message.reply_text("✅ Route 1 source channel set.")

async def set_destination_channel1(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ROUTES["1"]["destination"] = get_chat_id(update)
    save_config()
    await update.message.reply_text("✅ Route 1 destination channel set.")

async def set_caption1(update: Update, context: ContextTypes.DEFAULT_TYPE):
    caption = " ".join(context.args)
    if caption:
        ROUTES["1"]["caption"] = caption
        save_config()
        await update.message.reply_text("✅ Route 1 caption set.")
    else:
        await update.message.reply_text("⚠️ Please provide a caption.")

async def reset_route1(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ROUTES["1"] = {"source": "", "destination": "", "caption": ""}
    save_config()
    await update.message.reply_text("♻️ Route 1 reset.")

# Route 2
async def set_source_channel2(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ROUTES["2"]["source"] = get_chat_id(update)
    save_config()
    await update.message.reply_text("✅ Route 2 source channel set.")

async def set_destination_channel2(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ROUTES["2"]["destination"] = get_chat_id(update)
    save_config()
    await update.message.reply_text("✅ Route 2 destination channel set.")

async def set_caption2(update: Update, context: ContextTypes.DEFAULT_TYPE):
    caption = " ".join(context.args)
    if caption:
        ROUTES["2"]["caption"] = caption
        save_config()
        await update.message.reply_text("✅ Route 2 caption set.")
    else:
        await update.message.reply_text("⚠️ Please provide a caption.")

async def reset_route2(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ROUTES["2"] = {"source": "", "destination": "", "caption": ""}
    save_config()
    await update.message.reply_text("♻️ Route 2 reset.")

# Route 3
async def set_source_channel3(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ROUTES["3"]["source"] = get_chat_id(update)
    save_config()
    await update.message.reply_text("✅ Route 3 source channel set.")

async def set_destination_channel3(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ROUTES["3"]["destination"] = get_chat_id(update)
    save_config()
    await update.message.reply_text("✅ Route 3 destination channel set.")

async def set_caption3(update: Update, context: ContextTypes.DEFAULT_TYPE):
    caption = " ".join(context.args)
    if caption:
        ROUTES["3"]["caption"] = caption
        save_config()
        await update.message.reply_text("✅ Route 3 caption set.")
    else:
        await update.message.reply_text("⚠️ Please provide a caption.")

async def reset_route3(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ROUTES["3"] = {"source": "", "destination": "", "caption": ""}
    save_config()
    await update.message.reply_text("♻️ Route 3 reset.")

# View all routes
async def view_all_setups(update: Update, context: ContextTypes.DEFAULT_TYPE):
    msg = "🔧 Current Route Configurations:\n"
    for route_id, config in ROUTES.items():
        msg += f"\nRoute {route_id}:\n"
        msg += f"• Source: {config['source'] or '❌ Not Set'}\n"
        msg += f"• Destination: {config['destination'] or '❌ Not Set'}\n"
        msg += f"• Caption: {config['caption'] or '❌ Not Set'}\n"
    await update.message.reply_text(msg)

async def handle_channel_post(update: Update, context: ContextTypes.DEFAULT_TYPE):
    message = update.channel_post
    if not message or not message.document or not message.document.file_name.endswith(".apk"):
        return
    ...
    for route, data in ROUTES.items():
        try:
            src_channel = data.get("source", "").lstrip("@")
            dst_channel = data.get("destination", "")
            cap_template = data.get("caption", "")

            is_valid_source = (
                update.effective_chat.username == src_channel or
                str(update.effective_chat.id).endswith(src_channel.replace("-100", ""))
            )
            ...
            if (
                (route == "1" and 1 <= file_size_mb <= 50) or
                (route == "2" and 50 < file_size_mb <= 2048) or
                (route == "3")
            ):
                missing = []
                if not data.get("source"): missing.append(f"/sourcechannel{route}")
                if not data.get("destination"): missing.append(f"/desichannel{route}")
                if "Key -" not in cap_template: missing.append(f"/setcaption{route}")

                if missing:
                    await context.bot.send_message(
                        chat_id=OWNER_ID,
                        text=f"⚠️ Missing setup for Route {route}:\n" + "\n".join(missing)
                    )
                    return

                final_caption = cap_template.replace("Key -", f"Key - `{key}`")
                ...
                await context.bot.send_message(
                    ...
                    text=(
                        f"*Completed Process! :*\n"
                        f" • Source Picked → {data['source']}\n"
                        f" • Desi Sended → {data['destination']}\n"
                        f" • Sended Key → {key}"
                    ),
                    parse_mode="Markdown"
                )
            ...
            await context.bot.send_message(OWNER_ID, f"❗ Route {route} failed:\n{e}")
...
def main():
    ...
    app.add_handler(CommandHandler("sourcechannel1", set_source_channel1))
    app.add_handler(CommandHandler("desichannel1", set_destination_channel1))
    app.add_handler(CommandHandler("setcaption1", set_caption1))
    app.add_handler(CommandHandler("resetsetup1", reset_route1))
    app.add_handler(CommandHandler("sourcechannel2", set_source_channel2))
    app.add_handler(CommandHandler("desichannel2", set_destination_channel2))
    app.add_handler(CommandHandler("setcaption2", set_caption2))
    app.add_handler(CommandHandler("resetsetup2", reset_route2))
    app.add_handler(CommandHandler("sourcechannel3", set_source_channel3))
    app.add_handler(CommandHandler("desichannel3", set_destination_channel3))
    app.add_handler(CommandHandler("setcaption3", set_caption3))
    app.add_handler(CommandHandler("resetsetup3", reset_route3))
    app.add_handler(CommandHandler("viewsetup", view_all_setups))
    app.add_handler(MessageHandler(filters.ChatType.CHANNEL & filters.Document.ALL, handle_channel_post))
