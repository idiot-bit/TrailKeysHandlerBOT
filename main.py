...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Handle shared key for multiple APKs
    if state["status"] == "waiting_key":
        ...
        # Use pending_apks if multi-upload
        pending_apks = state.get("pending_apks", [])
        if pending_apks:
            for apk in pending_apks:
                apk["key"] = key
            ...
            await update.message.reply_text(f"✅ Key added to {count} APKs.", reply_markup=get_apk_share_keyboard())
        else:
            # Fallback to single APK mode
        ...
        if "Key -" not in caption_text:
            ...
            return
        USER_DATA[str(user_id)]["caption"] = caption_text
        save_config()
        USER_STATE[user_id]["status"] = "waiting_apk"
