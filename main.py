...
async def monitor_apks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for setup_number, setup in SETUPS.items():
        print(f"[Monitor] Incoming from {incoming_chat_id}, checking setup {setup_number}")
        ...
        try:
            ...
            await context.bot.send_message(
                OWNER_ID,
                f"*Completed Process! :*\n"
                f"• Source Picked → `{configured_source}`\n"
            ...
            await context.bot.send_message(
                OWNER_ID,
                f"❌ Error sending document (setup {setup_number}): `{e}`",
                parse_mode="Markdown"
            )

        return  # Only process one matching setup
