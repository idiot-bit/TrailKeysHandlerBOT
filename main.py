...
async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Init key tracking for this batch
    state.setdefault("incoming_keys", set())
    ...
    # Check if caption contains a key
    match = re.search(r'Key\s*-\s*(\S+)', caption)
    if match:
        detected_key = match.group(1)
        state["incoming_keys"].add(detected_key)

        if len(state["incoming_keys"]) > 1:
            # Conflict: multiple different keys detected, fallback to batch mode
            state["incoming_keys"] = set()  # Clear keys
            match = None  # Treat as keyless
        else:
            # Only one key detected so far
    ...
    if not match:
        # Key missing or rejected due to key mismatch — use batch flow
