...
async def process_method2_apk(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"✅ *APK {len(session)} received!*\n\n"
        "_Waiting for next APK (5 sec window)..._",
        parse_mode="Markdown"
    )

    # Start a wait/check
    context.application.create_task(check_session_timeout(user_id, context))
...
async def method2_send_to_channel(user_id, context):
    ...
    for idx, file_id in enumerate(session_files, start=1):
        ...
        elif idx == 1 and len(session_files) >= 2:
            caption = f"Key - <code>{key}</code>"
        elif idx == 2 and len(session_files) >= 2:
            caption = f"Key - <code>{key}</code>"
        elif idx == 3:
            caption = saved_caption.replace("Key -", f"Key - <code>{key}</code>")
...
async def method2_convert_quote(user_id, context):
    ...
    await context.bot.send_message(chat_id=user_id, text="✅ *Key converted to Quote Style!*", parse_mode="Markdown")
...
async def method2_convert_mono(user_id, context):
    ...
    await context.bot.send_message(chat_id=user_id, text="✅ *Key converted to Mono Style!*", parse_mode="Markdown")
