...
async def method2_send_to_channel(user_id, context):
    ...
    key_mode = state.get("key_mode", "normal")  # Default simple normal
    ...
    if last_message:
        ...
        buttons = [
            ...
            [InlineKeyboardButton("✏️ Auto Re-Caption", callback_data="auto_recaption")],
...
async def auto_recaption(user_id, context):
    ...
    old_post_id = state.get("last_post_id")  # Store old media group ID
    ...
    # Delete old wrong post
    try:
        await context.bot.delete_message(chat_id=channel_id, message_id=old_post_id)
    except:
        pass  # Ignore if already deleted

    # Save new last post ID
    new_last_msg = new_posts[-1]
    post_link = f"https://t.me/c/{channel_id.replace('-100', '')}/{new_last_msg.message_id}"

    USER_STATE[user_id]["last_post_link"] = post_link
    USER_STATE[user_id]["last_post_id"] = new_posts[0].message_id
