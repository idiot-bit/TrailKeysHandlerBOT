...
USER_STATE[user_id] = {
    "apks": [],  # store up to 3 APKs
    "status": "waiting_apk",
    "current_apk": None,  # temporary while waiting for key/caption
    "edit_mode": False
}
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(state["apks"]) == 0:
        ...
        if "Key -" not in user_caption:
            await update.message.reply_text(
                "📝 *Default caption missing `Key -` placeholder.*\nUse /setcaption to fix.",
                parse_mode="Markdown"
            )
            return
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_channel = USER_DATA.get(str(user_id), {}).get("channel")
    if not user_channel:
        await query.edit_message_text("⚠️ You haven't set a channel.\nUse `/setchannelid` first.")
        return

    for apk in USER_STATE[user_id]["apks"]:
        try:
            ...
            if str(user_channel).startswith("@"):
                post_link = f"https://t.me/{user_channel.strip('@')}/{sent.message_id}"
            else:
                post_link = f"https://t.me/c/{user_channel.replace('-100', '')}/{sent.message_id}"
    ...
    USER_STATE[user_id] = {"apks": [], "status": "waiting_apk", "current_apk": None}
        ...
        preview = ""
        for i, apk in enumerate(USER_STATE[user_id]["apks"], 1):
